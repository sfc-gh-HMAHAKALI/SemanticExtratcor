...
The ResMed SPU model carries 19,921 ``securityFilterDefinition`` entries. Each
binds one Cognos account to a row filter, almost always of the form::

    KAM-SPU.ROLE = 'KAM-SPU' AND KAM-SPU.TERRITORY_LEVEL4 = 'EAST.GREATLAKES.KAM.CLEVELAND'
...
ResMed's security model: hierarchical inheritance (an L2 manager seeing their L1
        ...
        out.append(
            {
                ...
                # The Cognos group path is the role hierarchy: SALES:SPU:KAM-SPU
...
def summarize_security(parsed: dict) -> dict[str, Any]:
    ...
    return {
        ...
        "hierarchy_warning": (
            "These filters are flat per-principal equality predicates. If ResMed "
...
{note}  EXISTS (
  ...
  OR IS_ROLE_IN_SESSION('SPU_SECURITY_ADMIN');
"""
