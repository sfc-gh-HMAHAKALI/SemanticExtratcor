...
export is large -- the ResMed SPU model is 18MB with 57,763 ``refobj`` nodes and
...
def map_cognos_datatype(
    ...
    alone misclassifies 168 of the 285 date columns in the ResMed SPU model as
...
# "CAMID(":SALES:SPU:KAM-SPU:EAST_GREATLAKES_KAM_CLEVELAND")"
...
def _parse_security_filter(sf: ET.Element) -> dict[str, Any]:
    ...
    Each definition binds a Cognos account/group to a row filter. In the ResMed
...
def _parse_package(pkg: ET.Element) -> dict[str, Any]:
    ...
    # "_CORP_NATIONAL_AER [Directory > Cognos > SALES > SPU > NAM-SPU]"
