...
2. **Relative-time reduction** -- the part that actually matters for this model.
   ResMed's 248 calculations do not compute date windows arithmetically. They
...
# Members that depend on ResMed's fiscal calendar rather than the Gregorian one.
#
# These are deliberately NOT given CURRENT_DATE() arithmetic. ResMed's fiscal year
...
def extract_scalar_tail(expression: str) -> str:
    ...
    Worth noting because it is easy to misread: the ResMed model's
...
def translate_relative_time_calculation(
    ...
    Handles the two shapes that account for the whole ResMed calculation
    library:
