...
- Core deps: `lkml`, `requests`, `pyyaml`, `openpyxl`, `pbixray` (for .pbix — ships prebuilt wheels, no C compiler needed)
- Optional deps: `jaydebeapi` + JVM (for Denodo JDBC)
- Fallback for .pbix: If `pbixray` fails, the skill auto-detects `pbi-tools` CLI on PATH (requires Power BI Desktop)
...
| `report` | Generate HTML assessment + Excel workbook | `python3 -m modules.cli report inventory.json -o ./output/ --customer Acme` |
...
5. **Seed data SQL** — synthetic INSERT/GENERATOR statements for populating Snowflake tables to support semantic view testing
6. **SI agent artifacts** — Cortex Agent specification JSON, semantic view DDL per domain, and deployment SQL script
